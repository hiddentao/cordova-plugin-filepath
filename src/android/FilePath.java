...
public class FilePath extends CordovaPlugin {
   ...
   * @param callbackContext The callback context through which to return stuff to
   *                        caller.
  ...
  public void resolveNativePath() throws JSONException {
    ...
    if (filePath == GET_PATH_ERROR_ID) {
      resultObj.put("code", checkPermission());
  ...
  /**
   * Get content:// from segment list In the new Uri Authority of Google Photos,
   * the last segment is not the content:// anymore So let's iterate through all
   * segments and find the content uri!
   *
   ...
   * file-based ContentProviders.<br>
   * <br>
   * Callers should check whether the path is local before assuming it represents
   * a local file.
   *
   * @param context The context.
   * @param uri     The Uri to query.
  ...
  private static String getPath(final Context context, final Uri uri) {
    ...
    Log.d(TAG,
        "File - " + "Authority: " + uri.getAuthority() + ", Fragment: " + uri.getFragment() + ", Port: " + uri.getPort()
    ...
    if (isKitKat && DocumentsContract.isDocumentUri(context, uri)) {
      ...
      // DownloadsProvider
        ...
        // thanks to
        // https://github.com/hiddentao/cordova-plugin-filepath/issues/34#issuecomment-430129959
        Cursor cursor = null;
        try {
          cursor = context.getContentResolver().query(uri, new String[] { MediaStore.MediaColumns.DISPLAY_NAME }, null,
              null, null);
          if (cursor != null && cursor.moveToFirst()) {
  ...
  private static String getDriveFilePath(Uri uri, Context context) {
    ...
    /*
     * Get the column indexes of the data in the Cursor, * move to the first row in
     * the Cursor, get the data, * and display it.
     */
